...
function Register() {
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      ...
      navigate("/login", {
        state: { successMessage: "Cuenta creada correctamente. Ahora podes iniciar sesion." },
      });
    } catch (err) {
      const message =
        err.response?.data?.error?.message ??
        "Ocurrio un error al crear la cuenta. Intenta nuevamente.";
    ...
    <section className="w-full max-w-[420px] mx-auto my-12 bg-white md:rounded-xl shadow-xl overflow-hidden flex flex-col">
      ...
      >
        <div className="space-y-1.5">
          <label htmlFor="name" className="text-sm font-medium text-on-surface-variant ml-1">
            Nombre completo
          </label>
        ...
        <div className="space-y-1.5">
          <label htmlFor="email" className="text-sm font-medium text-on-surface-variant ml-1">
            Correo electronico
          </label>
          <div className="relative">
            <span
        ...
        <div className="space-y-1.5">
          <label htmlFor="password" className="text-sm font-medium text-on-surface-variant ml-1">
            Contraseña
          </label>
          <div className="relative">
            <span
        ...
        <div className="space-y-1.5">
          ...
          {errors.confirmPassword && (
            <p id="confirm-password-error" className="text-xs text-error ml-1">
